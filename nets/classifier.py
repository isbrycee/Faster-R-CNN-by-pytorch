...
class Resnet50RoIHead(nn.Module):
    ...
    def forward(self, x, rois, roi_indices):
        ...
        # 利用建议框对公用特征层进行截取
        pool = self.roi(x, indices_and_rois)
        # print('feature_map:', np.shape(x))  # [1, 1024, 38, 38]
        # print('Rois:', np.shape(rois))  # [128, 4] 128个候选区域
        # print('After ROIPooling:', np.shape(pool))  # [128, 1024, 14, 14]    由38大小pooling成14
        fc7 = self.classifier(pool)  # ResNet50的  self.avgpool = nn.AvgPool2d(7)
        # print('After classifier:', np.shape(fc7))  # [128, 2048, 1, 1] )
        fc7 = fc7.view(fc7.size(0), -1)
        # print('After classifier Flatten:', np.shape(fc7))       # [128, 2048]
        roi_cls_locs = self.cls_loc(fc7)  # [128, 84]
        roi_scores = self.score(fc7)  # [128, 21]
        # print('roi_cls_locs', roi_cls_locs)
        # print('roi_cls_locs', np.shape(roi_cls_locs))
