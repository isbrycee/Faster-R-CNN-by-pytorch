...
class ProposalCreator():
    ...
    def __call__(self, loc, score,
        ...
        roi[:, slice(0, 4, 2)] = np.clip(roi[:, slice(0, 4, 2)], 0, img_size[1])  # 去掉超出图像宽的框
        roi[:, slice(1, 4, 2)] = np.clip(roi[:, slice(1, 4, 2)], 0, img_size[0])  # 去掉超出图像高的框
        ...
        score = score[keep]
        # 取出成绩最好的一些建议框
        order = score.ravel().argsort()[::-1]  # 升序排列后倒着取，结果即为降序
        if n_pre_nms > 0:
...
class RegionProposalNetwork(nn.Module):
    ...
    def forward(self, x, img_size, scale=1.):
        ...
        rpn_fg_scores = rpn_softmax_scores[:, :, 1].contiguous()  # 获取是目标的概率且在内存中连续排列，方便reshape
